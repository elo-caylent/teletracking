...
resource "aws_networkfirewall_rule_group" "fivetuple_stateful_group" {
  ...
  rule_group {
    ...
    dynamic "rule_variables" {
      for_each = [
        for b in lookup(var.fivetuple_stateful_rule_group[count.index], "rule_variables", {}) : b
        if length(b) > 1
      ]

      content {
        dynamic "ip_sets" {
          for_each = lookup(lookup(var.fivetuple_stateful_rule_group[count.index], "rule_variables", {}), "ip_sets", [])
          content {
            key = ip_sets.value["key"]
            ip_set {
              definition = ip_sets.value["ip_set"]
        ...
        dynamic "port_sets" {
          for_each = lookup(lookup(var.fivetuple_stateful_rule_group[count.index], "rule_variables", {}), "port_sets", [])
          content {
            key = port_sets.value["key"]
            port_set {
              definition = port_sets.value["port_sets"]
